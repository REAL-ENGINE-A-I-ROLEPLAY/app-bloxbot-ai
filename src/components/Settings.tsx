...
type SettingsTab = "providers" | "models" | "automation" | "appearance" | "privacy" | "about";
...
function AutomationTab() {
  ...
  const modeOptions: Array<{ value: AutonomyMode; label: string; detail: string }> = [
    {
      value: "suggest",
      label: "Suggest",
      detail: "Plans only, always waits for approval before risky actions.",
    },
    {
      value: "semi_auto",
      label: "Semi-Auto",
      detail: "Executes tasks with approval checkpoints for risky actions.",
    },
    {
      value: "full_auto",
      label: "Full Auto",
      detail: "Executes end-to-end while enforcing safety policy gates.",
    },
  ];

  return (
    <div className="mx-auto w-full max-w-md px-6 py-8">
      <h4 className="font-serif text-lg italic text-foreground">Automation</h4>
      <p className="mt-1 text-xs text-muted-foreground">
        Configure the policy-driven autonomous execution system for Roblox workflows.
      </p>

      <div className="mt-6 space-y-2">
        {modeOptions.map((option) => {
          const selected = autonomyMode === option.value;
          return (
            <button
              key={option.value}
              type="button"
      ...
      <div className="mt-6 rounded-lg border bg-card p-3.5">
        <div className="text-sm font-medium">Safety policy</div>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-[11px] text-muted-foreground">
          <li>Backups required: {autonomySettings.safetyPolicy.requireBackups ? "On" : "Off"}</li>
          <li>
            Rollback points required:{" "}
            {autonomySettings.safetyPolicy.requireRollbackPoints ? "On" : "Off"}
          </li>
          <li>
            Validation required: {autonomySettings.safetyPolicy.requireValidation ? "On" : "Off"}
          </li>
          <li>
            Documentation required:{" "}
            {autonomySettings.safetyPolicy.requireDocumentation ? "On" : "Off"}
          </li>
        </ul>
      </div>
      ...
      <div className="mt-6 rounded-lg border bg-card p-3.5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-sm font-medium">Don't ask ownership/license prompts again</div>
            <p className="mt-1 text-[11px] text-muted-foreground">
              Auto-applies stored ownership/licensing preference rules when possible.
            </p>
          </div>
          <button
            type="button"
            role="switch"
            aria-checked={autonomySettings.dontAskOwnershipAgain}
            onClick={() => setDontAskOwnershipAgain(!autonomySettings.dontAskOwnershipAgain)}
            className={`relative mt-0.5 inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${
              autonomySettings.dontAskOwnershipAgain ? "bg-foreground" : "bg-border"
            }`}
          >
            <span
              className={`inline-block h-3.5 w-3.5 rounded-full bg-background transition-transform ${
                autonomySettings.dontAskOwnershipAgain ? "translate-x-4" : "translate-x-0.5"
              }`}
            />
          </button>
        </div>
      ...
      <div className="mt-6">
        <label className="mb-1.5 block px-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
          Workplace scope key
        </label>
        <input
          type="text"
          value={autonomySettings.workspaceScopeKey}
          onChange={(event) => setWorkspaceScopeKey(event.target.value || "local-workspace")}
          className="h-9 w-full rounded-lg border bg-card px-3 text-xs"
